...
namespace Modules\ReqMerried\Http\Controllers;
...
use Modules\ReqMerried\Repositories\ReqMerriedRepository;
...
class ReqMerriedController extends Controller
{
    public function __construct(){
        ...
        $this->_reqMerriedRepository        = new ReqMerriedRepository;
    ...
    public function create(Request $request)
    {
        ...
        return view('reqmerried::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request['id_status_kawin']    = $user->id_merried_status;
         ...
         //new subdomain
         $request['service_id']         = 'REQ_MERRIED';
         ...
         $this->_reqMerriedRepository->insert(DataHelper::_normalizeParams($request->all(), true));
         ...
         if($req_id != null){
            $data_email   = array(
                ...
                'url'           => env('APP_URL').'/user/layanan/belum-menikah/lihat/'.$req_id,
                'service_name'  => 'Surat Keterangan Belum Menikah',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_reqMerriedRepository->getByParams($filter); 

        return view('reqmerried::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqmerried::edit', compact ('request', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request['id_status_kawin']    = $user->id_merried_status;
         ...
         $this->_reqMerriedRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_merried/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_merried/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_kk') != '') {
            ...
            $request->file("f_kk")->storeAs('files/request_merried/kk', $imageName, 'public');
        ...
        if ($request->file('f_pernyataan') != '') {
            ...
            $request->file("f_pernyataan")->storeAs('files/request_merried/surat_pernyataan', $imageName, 'public');
        ...
        if ($request->file('f_akta') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_akta->extension(); 

            $request->file("f_akta")->storeAs('files/request_merried/akta_cerai', $imageName, 'public');
