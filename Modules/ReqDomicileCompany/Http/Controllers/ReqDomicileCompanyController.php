...
namespace Modules\ReqDomicileCompany\Http\Controllers;
...
use Modules\ReqDomicileCompany\Repositories\DomicileCompanyRepository;

use App\Helpers\DataHelper;
use App\Helpers\LogHelper;
use DB;

class ReqDomicileCompanyController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('reqdomicilecompany::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('reqdomicilecompany::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $tgl_akta_notaris = str_replace('/', '-', $request->tgl_akta_notaris);
        $request['tgl_akta_notaris'] = date("Y-m-d", strtotime($tgl_akta_notaris));
        ...
        //new subdomain
        $request['service_id']         = 'REQ_COMPANY';
        ...
        $this->_domicileCompanyRepository->insert(DataHelper::_normalizeParams($request->all(), true));
        ...
        if($req_id != null){
            $data_email   = array(
                ...
                'url'           => env('APP_URL').'/user/layanan/domisili-perusahaan/lihat/'.$req_id,
                'service_name'  => 'Surat Keterangan Domisili Perusahaan',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_domicileCompanyRepository->getByParams($filter); 

        return view('reqdomicilecompany::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqdomicilecompany::edit', compact ('request', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->_domicileCompanyRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_company/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_company/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_pendirian') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_pendirian->extension(); 

            $request->file("f_pendirian")->storeAs('files/request_company/pendirian', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_PENDIRIAN');

        } 
        
        if ($request->file('f_sewa_kepemilikan') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_sewa_kepemilikan->extension(); 

            $request->file("f_sewa_kepemilikan")->storeAs('files/request_company/sewa_kepemilikan', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_SEWA_KEPEMILIKAN');

        } 
        
        if ($request->file('f_keterangan_rumah') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_keterangan_rumah->extension(); 

            $request->file("f_keterangan_rumah")->storeAs('files/request_company/keterangan_rumah', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_KETERANGAN_RUMAH');
