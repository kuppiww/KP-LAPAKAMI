...
<div class="row mb-4 align-items-center">
	...
	<div class="col-md-6 text-md-end">
		@if ($request->request_status_id == 'SUBMITED' || $request->request_status_id == 'REJECTED')
			<a href="/user/layanan/penduduk/ubah/{{ $request->request_id }}" class="btn btn-warning">
				<i class="ri-pencil-line me-2"></i> Ubah Permohonan
			</a>
		@endif
		@if ($request->request_status_id == 'SUBMITED')
			<a href="javascript:void(0)" data-url="/user/layanan/penduduk/batal/{{ $request->request_id }}" class="btn btn-danger btnCancelReq">
								...
								<div class="col-md-6">
									<p>
										@if ($request_doc->request_attachment_note == "FILE_KTP")
											...
											<a href="javascript:void(0)" data-title="Kartu Tanda Penduduk (KTP)" data-src="/storage/files/request_resident/ktp/{{$request_doc->request_attachment_file}}" class="btn btn-light btnStreamModal"><i class="ri-image-line me-2"></i> KTP</a>
										@elseif ($request_doc->request_attachment_note == "FILE_RT_RW")
											<label class="form-label">Surat Pengantar RT/RW</label><br>
											<a href="javascript:void(0)" data-title="Surat Pengantar RT/RW" data-src="/storage/files/request_resident/pengantar_rt_rw/{{$request_doc->request_attachment_file}}" class="btn btn-light btnStreamModal"><i class="ri-image-line me-2"></i> Surat Pengantar</a>
										@elseif ($request_doc->request_attachment_note == "FILE_KK")
											<label class="form-label">Kartu Keluarga</label><br>
											<a href="javascript:void(0)" data-title="Kartu Keluarga" data-src="/storage/files/request_resident/kk/{{$request_doc->request_attachment_file}}" class="btn btn-light btnStreamModal"><i class="ri-image-line me-2"></i> Kartu Keluarga</a>
										@elseif ($request_doc->request_attachment_note == "FILE_PENDUKUNG")
											<label class="form-label">Dokumen Pendukung</label><br>
											<a href="javascript:void(0)" data-title="Dokumen Pendukung" data-src="/storage/files/request_resident/pendukung/{{$request_doc->request_attachment_file}}" class="btn btn-light btnStreamModal"><i class="ri-image-line me-2"></i> Dokumen Pendukung</a>
										@endif
									</p>
								</div>
							@endforeach
