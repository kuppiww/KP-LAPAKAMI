<?php

namespace Modules\ReqResident\Repositories;

use App\Implementations\QueryBuilderImplementation;
use Illuminate\Support\Facades\DB;

class ResidentRepository extends QueryBuilderImplementation
{

    protected $fillable = ['kewarganegaraan', 'id_status_kawin', 'masa_berlaku', 'request_id', 'created_at', 'created_by', 'updated_at', 'updated_at'];

    public function __construct()
    {
        $this->table = 'req_ket_residents';
        $this->pk = 'req_ket_resident_id';
    }

    //overide
    public function insert(array $data)
    {
        $data['requests_det']['kewarganegaraan'] = $data['kewarganegaraan'];
        $data['requests_det']['id_status_kawin'] = $data['id_status_kawin'];
        $data['requests_det']['request_id']      = $data['request_id'];
        $data['requests_det']['created_at']      = $data['created_at'];
        $data['requests_det']['created_by']      = $data['created_by'];

        try {
            DB::beginTransaction();
                DB::table($this->table)->insert($data['requests_det']);
            DB::commit();
        } catch (Exception $e) {
            DB::rollback();
            return $e->getMessage();
        }
    }

    //overide
    public function update(array $data, $id)
    {
        // $data['requests_det']['kewarganegaraan'] = $data['kewarganegaraan'];
        // $data['requests_det']['id_status_kawin'] = $data['id_status_kawin'];
        $data['requests_det']['request_id']      = $data['request_id'];
        $data['requests_det']['updated_at']      = $data['updated_at'];
        $data['requests_det']['updated_by']      = $data['updated_by'];

        try {
            DB::beginTransaction();
                DB::table($this->table)
                ->where('request_id', '=', $id)
                ->update($data['requests_det']);
            DB::commit();
        } catch (Exception $e) {
            DB::rollback();
            return $e->getMessage();
        }
    }

    // overide
    public function getByParams(array $params)
    {
        try {
            return DB::connection($this->db)
                ->table($this->table)
                ->select('req_ket_residents.*', 'merried_status')
                ->join('m_merried_status', 'm_merried_status.id_merried_status', '=', 'req_ket_residents.id_status_kawin')
                ->where($params)
                ->first();
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
}