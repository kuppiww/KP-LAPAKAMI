...
namespace Modules\ReqResident\Http\Controllers;
...
use Modules\ReqResident\Repositories\ResidentRepository;

use App\Helpers\DataHelper;
use App\Helpers\LogHelper;
use DB;

class ReqResidentController extends Controller
{
    public function __construct(){
        ...
        $this->_residentRepository          = new ResidentRepository;
    ...
    public function create(Request $request)
    {
        ...
        return view('reqresident::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        //new subdomain
        $request['service_id']         = 'REQ_RESIDENT';
        ...
        $this->_residentRepository->insert(DataHelper::_normalizeParams($request->all(), true));
        ...
        if($req_id != null){
            $data_email   = array(
                ...
                'service_name'  => 'Surat Keterangan Penduduk',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_residentRepository->getByParams($filter); 

        return view('reqresident::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqresident::edit', compact ('request', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->_residentRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_resident/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_resident/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_kk') != '') {
            ...
            $request->file("f_kk")->storeAs('files/request_resident/kk', $imageName, 'public');
        ...
        if ($request->file('f_pendukung') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_pendukung->extension(); 

            $request->file("f_pendukung")->storeAs('files/request_resident/pendukung', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_PENDUKUNG');
