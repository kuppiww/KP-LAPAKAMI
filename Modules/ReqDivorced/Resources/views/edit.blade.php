...
@section('content')
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-md-12">
            <h4 class="text-dark fw-bold mb-0">Ubah Permohonan Layanan</h4>
            <p class="mb-0 text-muted">Surat Keterangan Janda/Duda</p>
        </div>
    </div>
    ...
    <div class="card p-2 border-0">
        <div class="card-body">
            ...
            <form id="createForm" method="POST" action="/user/layanan/janda-duda/update/{{ $request->request_id }}"
                ...
                <fieldset>
                    <div class="fieldset-content">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3 form-group">
                                <label class="form-label">Status Janda/Duda<span class="text-danger">*</span></label>
                                <select class="form-control form-select" name="id_status_janda">
                                    <option value="">- Pilih Status -</option>
                                    @if (sizeof($widow_stats) > 0)
                                        @foreach ($widow_stats as $widow_stat)
                                            <option value="{{ $widow_stat->id_widow_status }}"
                                                {{ $request_detail->id_status_janda == $widow_stat->id_widow_status ? 'selected' : '' }}>
                                                {{ $widow_stat->widow_status }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="col-md-6 mb-3 form-group">
                                <label class="form-label">Sebab Janda/Duda<span class="text-danger">*</span></label>
                                <select class="form-control form-select" name="id_ket">
                                    <option value="">- Pilih Sebab -</option>
                                    @if (sizeof($widow_kets) > 0)
                                        @foreach ($widow_kets as $widow_ket)
                                            <option value="{{ $widow_ket->id_ket }}"
                                                {{ $request_detail->id_ket == $widow_ket->id_ket ? 'selected' : '' }}>
                                                {{ $widow_ket->ket }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="col-md-12 mb-3 form-group">
                                ...
                                <input type="text" name="perihal" value={{ $request_detail->perihal }}
                ...
                <fieldset>
                    <div class="fieldset-content">
                        <div class="row">
                                ...
                                <label class="form-label">Kartu Tanda Penduduk (KTP)<span class="text-danger">*</span>
                                    @foreach ($request_docs as $request_doc)
                                        @if ($request_doc->request_attachment_note == 'FILE_KTP')
                                            <a href="/storage/files/request_devorced/ktp/{{ $request_doc->request_attachment_file }}"
                                                target="_blank"
                                ...
                                <label class="form-label">Kartu Keluarga<span class="text-danger">*</span>
                                    @foreach ($request_docs as $request_doc)
                                        @if ($request_doc->request_attachment_note == 'FILE_KK')
                                            <a href="/storage/files/request_devorced/kk/{{ $request_doc->request_attachment_file }}"
                                                target="_blank"
                                                class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
                                                <i class="ri-arrow-right-up-line"></i> Lihat File
                                            </a>
                                ...
                                <label class="form-label">Surat Pengantar RT/RW<span class="text-danger">*</span>
                                    @foreach ($request_docs as $request_doc)
                                        @if ($request_doc->request_attachment_note == 'FILE_RT_RW')
                                            <a href="/storage/files/request_devorced/pengantar_rt_rw/{{ $request_doc->request_attachment_file }}"
                                                target="_blank"
                                                class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
                                                <i class="ri-arrow-right-up-line"></i> Lihat File
                                            </a>
                            ...
                            <div class="col-md-6 mb-3 form-group">
                                <label class="form-label">Akta Cerai/Akta Kematian Suami/Istri<span
                                        class="text-danger">*</span>
                                    @foreach ($request_docs as $request_doc)
                                        @if ($request_doc->request_attachment_note == 'FILE_AKTA_CERAI')
                                            <a href="/storage/files/request_devorced/akta_cerai/{{ $request_doc->request_attachment_file }}"
                                                target="_blank"
                                                class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
                                                <i class="ri-arrow-right-up-line"></i> Lihat File
                                            </a>
                                ...
                                <input type="file" name="f_akta_cerai" class="form-control"
                                    placeholder="Masukan akta cerai">
                                <small class="form-note">File .jpg, .jpeg, .png, .pdf, Maksimal 1MB</small>
                            </div>
                            <div class="col-md-6 mb-3 form-group">
                                <label class="form-label">Surat Dibawah Tangan/SPTJM (bagi pemohon untuk nikah siri)
                                    @foreach ($request_docs as $request_doc)
                                        @if ($request_doc->request_attachment_note == 'FILE_SPTJM')
                                            <a href="/storage/files/request_devorced/sptjm/{{ $request_doc->request_attachment_file }}"
                                                target="_blank"
                                                class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
                                                <i class="ri-arrow-right-up-line"></i> Lihat File
                                            </a>
                                ...
                                <input type="file" name="f_sptjm" class="form-control"
                                    placeholder="Masukan surat dibawah tangan">
                                <small class="form-note">File .jpg, .jpeg, .png, .pdf, Maksimal 1MB</small>
                            </div>
    ...
    <script type="text/javascript">
        (function($) {
            ...
            form.validate({
                ...
                messages: {
                    id_status_janda: "Status janda duda harus dipilih",
                    id_ket: "Sebab janda duda harus dipilih",
                    perihal: "Keperluan tidak boleh kosong",
