...
namespace Modules\ReqDivorced\Http\Controllers;
...
use Modules\Request\Repositories\WidowRepository;
use Modules\Request\Repositories\WidowStatusRepository;
use Modules\Request\Repositories\RequestAttachmentRepository;
use Modules\Request\Repositories\ServiceRepository;
use Modules\ReqDivorced\Repositories\DivorcedRepository;

use App\Helpers\DataHelper;
use App\Helpers\LogHelper;
use DB;

class ReqDivorcedController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $widow_stats    = $this->_widowStatusRepository->getAll();
        $widow_kets     = $this->_widowRepository->getAll();
        $attach_samples = $this->_serviceRepository->getRequirementSample(['service_id' => $getService->service_id]);

        return view('reqdivorced::create',  compact('user', 'widow_stats', 'widow_kets', 'sub_districts', 'districts', 'religions', 'genders', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        //new subdomain
        $request['service_id']         = 'REQ_DIVORCED';
        ...
        $this->_divorcedRepository->insert(DataHelper::_normalizeParams($request->all(), true));
        ...
        if($req_id != null){
            $data_email   = array(
                ...
                'url'           => env('APP_URL').'/user/layanan/janda-duda/lihat/'.$req_id,
                'service_name'  => 'Surat Keterangan Janda Duda',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_divorcedRepository->getByParams($filter); 

        return view('reqdivorced::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqdivorced::edit', compact ('request', 'widow_stats', 'widow_kets', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->_divorcedRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_devorced/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_devorced/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_kk') != '') {
            ...
            $request->file("f_kk")->storeAs('files/request_devorced/kk', $imageName, 'public');
        ...
        if ($request->file('f_akta_cerai') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_akta_cerai->extension(); 

            $request->file("f_akta_cerai")->storeAs('files/request_devorced/akta_cerai', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_AKTA_CERAI');

        } 
        if ($request->file('f_sptjm') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_sptjm->extension(); 

            $request->file("f_sptjm")->storeAs('files/request_devorced/sptjm', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_SPTJM');
