...
namespace Modules\ReqCrowd\Http\Controllers;
...
use Modules\ReqCrowd\Repositories\CrowdRepository;
...
class ReqCrowdController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('reqcrowd::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $tgl_kegiatan = str_replace('/', '-', $request->tgl_kegiatan);
        $request['tgl_kegiatan'] = date("Y-m-d", strtotime($tgl_kegiatan));
        ...
        if(isset($request->tgl_kegiatan_akhir)){
            $tgl_kegiatan_akhir = str_replace('/', '-', $request->tgl_kegiatan_akhir);
            $request['tgl_kegiatan_akhir'] = date("Y-m-d", strtotime($tgl_kegiatan_akhir));
        }else{
            $request['tgl_kegiatan_akhir'] = $request['tgl_kegiatan'];
        }

        //new subdomain
        $request['service_id']         = 'REQ_CROWD';
        ...
        $this->_crowdRepository->insert(DataHelper::_normalizeParams($request->all(), true));
        ...
        if($req_id != null){
            $data_email   = array(
                ...
                'url'           => env('APP_URL').'/user/layanan/izin-keramaian/lihat/'.$req_id,
                'service_name'  => 'Surat Izin Keramaian',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_crowdRepository->getByParams($filter); 

        return view('reqcrowd::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqcrowd::edit', compact ('request', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->_crowdRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_crowd/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_crowd/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_kk') != '') {
            ...
            $request->file("f_kk")->storeAs('files/request_crowd/kk', $imageName, 'public');
        ...
        if ($request->file('f_pemberitahuan_tetangga') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_pemberitahuan_tetangga->extension(); 

            $request->file("f_pemberitahuan_tetangga")->storeAs('files/request_crowd/pemberitahuan_tetangga', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_PEMBERITAHUAN_TETANGGA');

        } 
        if ($request->file('f_proposal') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_proposal->extension(); 

            $request->file("f_proposal")->storeAs('files/request_crowd/proposal', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_PROPOSAL');

        } 
        if ($request->file('f_panitia') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_panitia->extension(); 

            $request->file("f_panitia")->storeAs('files/request_crowd/panitia', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_PANITIA');
