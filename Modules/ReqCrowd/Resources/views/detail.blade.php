    ...
    <div class="row mb-4 align-items-center">
        ...
        <div class="col-md-6 text-md-end">
            @if ($request->request_status_id == 'SUBMITED' || $request->request_status_id == 'REJECTED')
                <a href="/user/layanan/izin-keramaian/ubah/{{ $request->request_id }}" class="btn btn-warning">
                    <i class="ri-pencil-line me-2"></i> Ubah Permohonan
                </a>
            @endif
            @if ($request->request_status_id == 'SUBMITED')
                <a href="javascript:void(0)" data-url="/user/layanan/izin-keramaian/batal/{{ $request->request_id }}"
                    class="btn btn-danger btnCancelReq">
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        ...
                        <div class="tab-pane fade" id="pills-service" role="tabpanel" aria-labelledby="pills-profile-tab"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3">
                                    <h5 class="text-dark">Informasi Kegiatan</h5>
                                </div>
                                <div class="col-md-6">
                                    <p>
                                        <label class="form-label">Nama Kegiatan</label><br>
                                        {{ $request_detail->kegiatan }}
                                    </p>
                                </div>
                                <div class="col-md-6">
                                    <p>
                                        <label class="form-label">Tanggal Kegiatan</label><br>
                                        {{ DateFormatHelper::dateIn($request_detail->tgl_kegiatan) }} s/d
                                        {{ isset($request_detail->tgl_kegiatan_akhir) ? DateFormatHelper::dateIn($request_detail->tgl_kegiatan_akhir) : '-' }}
                                    </p>
                                </div>
                                <div class="col-md-6">
                                    <p>
                                        <label class="form-label">Waktu</label><br>
                                        {{ $request_detail->waktu . ' - ' . $request_detail->waktu_akhir }}
                                    </p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="col-md-6">
                                        <p>
                                            @if ($request_doc->request_attachment_note == 'FILE_KTP')
                                                ...
                                                <a href="javascript:void(0)" data-title="Kartu Tanda Penduduk (KTP)" data-src="/storage/files/request_crowd/ktp/{{ $request_doc->request_attachment_file }}"
                                                 class="btn btn-light btnStreamModal"><i
                                                        class="ri-image-line me-2"></i> KTP</a>
                                            @elseif ($request_doc->request_attachment_note == 'FILE_RT_RW')
                                                <label class="form-label">Surat Pengantar RT/RW</label><br>
                                                <a href="javascript:void(0)" data-title="Surat Pengantar RT/RW" data-src="/storage/files/request_crowd/pengantar_rt_rw/{{ $request_doc->request_attachment_file }}"
                                                     class="btn btn-light btnStreamModal"><i
                                            ...
                                            @elseif ($request_doc->request_attachment_note == 'FILE_KK')
                                                ...
                                                <a href="javascript:void(0)" data-title="Kartu Keluarga" data-src="/storage/files/request_crowd/kk/{{ $request_doc->request_attachment_file }}"
                                                     class="btn btn-light btnStreamModal"><i
                                                        class="ri-image-line me-2"></i> Kartu Keluarga</a>
                                            @elseif ($request_doc->request_attachment_note == 'FILE_PEMBERITAHUAN_TETANGGA')
                                                <label class="form-label">Surat Pemberitahuan Tetangga</label><br>
                                                <a href="javascript:void(0)" data-title="Surat Pemberitahuan Tetangga" data-src="/storage/files/request_crowd/pemberitahuan_tetangga/{{ $request_doc->request_attachment_file }}"
                                                     class="btn btn-light btnStreamModal"><i
                                                        class="ri-image-line me-2"></i> Surat Pemberitahuan Tetangga</a>
                                            @elseif ($request_doc->request_attachment_note == 'FILE_PROPOSAL')
                                                <label class="form-label">Proposal Kegiatan</label><br>
                                                <a href="javascript:void(0)" data-title="Proposal Kegiatan" data-src="/storage/files/request_crowd/proposal/{{ $request_doc->request_attachment_file }}"
                                                    class="btn btn-light btnStreamModal"><i
                                                        class="ri-image-line me-2"></i> Proposal kegiatan</a>
                                            @elseif ($request_doc->request_attachment_note == 'FILE_PROPOSAL')
                                                <label class="form-label">Surat Keterangan Panitia</label><br>
                                                <a href="javascript:void(0)" data-title="Surat Keterangan Panitia" data-src="/storage/files/request_crowd/panitia/{{ $request_doc->request_attachment_file }}"
                                                    class="btn btn-light btnStreamModal"><i
                                                        class="ri-image-line me-2"></i> Surat Keterangan Panitia</a>
                                            @endif
                                        </p>
                                    </div>
                                @endforeach
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <embed src="{{ url('backend/'. $request->slug_simkel .'/besign-pdf/'. $request->request_file) }}"
                        class="w-100" width="" height="500" type="application/pdf">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Tutup</button>
                    <a type="button" class="btn btn-primary"
                        href="{{ url('backend/'. $request->slug_simkel .'/besign-pdf/'. $request->request_file) }}"
                        download>Unduh</a>
                </div>
            </div>
        </div>
