...
<div class="card p-2 border-0">
	<div class="card-body">
		...
		<form id="createForm" method="POST" action="/user/layanan/skck/update/{{$request->request_id}}" enctype="multipart/form-data">
			@csrf
			<!-- Form 1 -->
			<h3>
                ...
                <div class="fieldset-content">
                	<div class="row">
						...
						<div class="col-md-12 mb-3 form-group">
							...
							<input type="text" name="keperluan" value="{{$request_detail->keperluan}}" class="form-control" placeholder="Masukan keperluan">
						</div>
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_KTP')
										<a href="/storage/files/request_skck/ktp/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_KK')
										<a href="/storage/files/request_skck/kk/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
											<i class="ri-arrow-right-up-line"></i> Lihat File
										</a>
									@endif
								@endforeach
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_RT_RW')
										<a href="/storage/files/request_skck/pengantar_rt_rw/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
											<i class="ri-arrow-right-up-line"></i> Lihat File
										</a>
									@endif
								@endforeach
