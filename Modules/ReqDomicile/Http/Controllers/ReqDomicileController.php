...
namespace Modules\ReqDomicile\Http\Controllers;
...
use Modules\ReqDomicile\Repositories\DomicileRepository;

use App\Helpers\DataHelper;
use App\Helpers\LogHelper;
use DB;

class ReqDomicileController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('reqdomicile::create',  compact('user', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function store(Request $request)
    {
        ...
        //new subdomain
        $request['service_id']         = 'REQ_DOMICILE';
        ...
        $this->_domicileRepository->insert(DataHelper::_normalizeParams($request->all(), true));
        ...
        if($req_id != null){
            $data_email   = array(
                ...
                'url'           => env('APP_URL').'/user/layanan/domisili/lihat/'.$req_id,
                'service_name'  => 'Surat Keterangan Domisili Yayasan, Organisasi atau Parpol',
    ...
    public function show(Request $request, $id)
    {
        ...
        $request_detail  = $this->_domicileRepository->getByParams($filter); 

        return view('reqdomicile::detail', compact ('request', 'request_detail', 'logs', 'request_docs'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return view('reqdomicile::edit', compact ('request', 'request_detail', 'logs', 'request_docs', 'sub_districts', 'districts', 'religions', 'genders', 'merried_stats', 'attach_samples'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->_domicileRepository->update(DataHelper::_normalizeParams($request->all(), false, true), $id);
    ...
    private function _uploadFile($request)
    {
        ...
        if ($request->file('f_ktp') != '') {
            ...
            $request->file("f_ktp")->storeAs('files/request_domicile/ktp', $imageName, 'public');
        ...
        if ($request->file('f_p_rt_rw') != '') {
            ...
            $request->file("f_p_rt_rw")->storeAs('files/request_domicile/pengantar_rt_rw', $imageName, 'public');
        ...
        if ($request->file('f_pendirian') != '') {
            ...
            $request->file("f_pendirian")->storeAs('files/request_domicile/pendirian', $imageName, 'public');
        ...
        if ($request->file('f_sewa_kepemilikan') != '') {
            ...
            $request->file("f_sewa_kepemilikan")->storeAs('files/request_domicile/sewa_kepemilikan', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_SEWA_KEPEMILIKAN');

        } if ($request->file('f_denah') != '') {

            $imageName = md5(time() . rand()) .'.'. $request->f_denah->extension(); 

            $request->file("f_denah")->storeAs('files/request_domicile/denah', $imageName, 'public');

            array_push($data['file'], $imageName );

            array_push($data['note'], 'FILE_DENAH');
