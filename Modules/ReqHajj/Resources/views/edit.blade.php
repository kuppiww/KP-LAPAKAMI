...
<div class="card p-2 border-0">
	<div class="card-body">
		...
		<form id="createForm" method="POST" action="/user/layanan/ibadah-haji/update/{{$request->request_id}}" enctype="multipart/form-data">
			@csrf
			<!-- Form 1 -->
			<h3>
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_KTP')
										<a href="/storage/files/request_hajj/ktp/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_KK')
										<a href="/storage/files/request_hajj/kk/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
											<i class="ri-arrow-right-up-line"></i> Lihat File
										</a>
									@endif
								@endforeach
							...
							<label class="form-label">
								...
								@foreach ($request_docs as $request_doc)
									@if ($request_doc->request_attachment_note == 'FILE_RT_RW')
										<a href="/storage/files/request_hajj/pengantar_rt_rw/{{$request_doc->request_attachment_file}}" target="_blank" class="bg-light ms-2 text-primary p-1 px-2 rounded-2 ms-2">
											<i class="ri-arrow-right-up-line"></i> Lihat File
										</a>
									@endif
								@endforeach
